...
CREATE INDEX event_geo_index ON public.events USING gist (location_point);
...
CREATE OR REPLACE FUNCTION public.nearby_events(lat double precision, long double precision)
 ...
 LANGUAGE sql
AS $function$
select id, created_at, title, description, date, location, image_uri, user_id,
st_y(location_point::geometry) as lat, st_x(location_point::geometry) as long,
